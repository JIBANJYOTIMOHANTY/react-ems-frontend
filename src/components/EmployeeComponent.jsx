...
function EmployeeComponent() {
    ...
    function pageTitle() {
        if (id) {
            return <h1 className="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white"> Update Employee
            </h1>
        } else {
            return <h1 className="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white"> Add Employee
            </h1>
        }
    }
                ...
                <div
                    ...
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form className="space-y-4 md:space-y-6" method="POST">
                            <div>
                                <label htmlFor="FirstName" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Enter Employee First Name</label>
                                <input type="text" value={firstName} name="FirstName" className={`bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${errors.firstName ? 'null' : ''}`} placeholder="Jiban Jyoti" onChange={handleFirstName} />
                            </div>
                            {errors.firstName && <div className='text-red-500' >{errors.firstName}</div>}

                            <div>
                                <label htmlFor="lastName" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Enter Employee Last Name</label>
                                <input type="text" value={lastName} name="lastName" className={`bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${errors.lastName ? 'null' : ''}`} placeholder="Mohanty" onChange={handleLastName} />
                            </div>
                            {errors.lastName && <div className='text-red-500' >{errors.lastName}</div>}

                            <div>
                                <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Enter Employee email </label>
                                <input type="email" value={email} name="email" id="email" placeholder="jibanjyotimohanty315@gmail.com" className={`bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${errors.email ? 'null' : ''}`} onChange={handleEmail} />
                            </div>
